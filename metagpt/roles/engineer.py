...
class Engineer(Role):
    ...
    @staticmethod
    def _parse_info_tasks(task_msg: Document, filename: str) -> list[str]:
        m = json.loads(task_msg.content)
        analysis_list = m.get("Integration Analysis")
        analysis = next((file for file in analysis_list if file["file"] == filename), None)
        return analysis.get("description"), analysis.get("constants")
    ...
    async def _act_sp_with_cr(self, review=False) -> Set[str]:
        ...
        for todo in self.code_todos:
            ...
            dependencies = {coding_context.task_doc.root_relative_path}
            if self.config.inc:
    ...
    async def _think(self) -> Action | None:
        ...
        write_plan_and_change_filters = any_to_str_set([FixBug])
    ...
    async def _new_code_actions(self):
        """Creates new code writing actions based on changes in tasks and source files.
        
        This function serves two main purposes:
        1. Processes changes in task files by:
           - Using RAG (Retrieval Augmented Generation) to find relevant code based on task descriptions
           - Creating coding contexts for each task
           - Handling potential conflicts between tasks
           
        2. Handles direct modifications to source files by:
           - Creating coding contexts with proper dependencies
           - Adding them to the todo list for code writing
        
        The workflow:
        1. Initializes FAISS vector database for RAG operations
        2. Determines if we're in bug fix mode (affects which files to process)
        3. Processes changed task files:
           - Extracts task descriptions and constants
           - Uses RAG to retrieve relevant code snippets
           - Creates coding contexts and documents
        4. Processes directly modified source files:
           - Creates coding contexts with dependencies
           - Adds to todo list
        5. Sets up the first todo item if any exist
        
        Key components used:
        - SimpleEngine: RAG engine for code retrieval
        - CodingContext: Holds context for code generation
        - WriteCode: Action for generating code
        - ProjectRepo: Manages project files and dependencies
        
        Returns:
            None. Updates self.code_todos and potentially sets a new todo action.
        """
        # Initialize RAG engine with FAISS index for code retrieval
        ...
        # Check if we're in bug fix mode - affects which files we process
        bug_fix = await self._is_fixbug()
        # Get relevant file lists based on mode
        ...
        # Process changes triggered by task modifications
        for filename in changed_task_files:
            ...
            for task_filename in task_list:
                # Extract task details and use RAG to find relevant code
                description, constants = self._parse_info_tasks(task_doc, task_filename)
                pattern = r"(.*?)\#\#The following is the explaination of this part of code:"
                nodes = await engine.aretrieve("What's the code relevant to the following description and constants: "+f"##description:({description}) ##constants:({str(constants)})")
                
                # Create documents and contexts for the task
                original_code = Document(root_path=str(self.project_repo.src_relative_path), filename=task_filename, content="####".join([re.search(pattern, node.text, re.DOTALL).group(1) for node in nodes]))
                context = CodingContext(
                    code_doc= original_code,
                ...
                # Log potential conflicts and update changed files
        ...
        # Set the first todo if we have any
        if self.code_todos:
