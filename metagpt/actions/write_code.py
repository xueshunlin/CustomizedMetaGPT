...
PROMPT_TEMPLATE = """
NOTICE
Role: You are a professional software engineer. Your primary goal is to implement a modular, maintainable, and scalable codebase derived from a previously monolithic Jupyter Notebook. You must adhere strictly to the legacy code and requirements provided, decomposing the notebook into logically organized Python files (modules) without introducing unrelated functionality.

Language: Follow the user's language requirements for the response text, but keep file names and code elements in English.

ATTENTION:
- Use '##' to SPLIT SECTIONS in your final output.
- Carefully follow the "Format example" provided, ensuring consistent formatting and documentation.
- Maintain the original logic of the notebook, reusing existing code where possible.
- Ensure each module can run independently and interact with others as needed.
- Develop a clear interface for modules to pass data and communicate errors.
- Document the purpose and functionality of each module, as well as all classes, functions, and constants.
...
4. Follow design: YOU MUST ADHERE to the defined data structures and interfaces; DO NOT introduce new public functions or alter existing designs.
5. CAREFULLY CHECK THAT YOU DONT MISS ANY NECESSARY CLASS/FUNCTION IN THIS FILE.
6. Before using a external variable/module, make sure you import it first.
7. Write out EVERY CODE DETAIL, DON'T LEAVE TODO.
8. Docstrings: Provide clear, concise docstrings for all classes, methods, and functions. Follow standard format:
For classes, describe the class's purpose and its attributes.
For methods and functions, describe parameters (with types), return values, exceptions raised, and overall functionality. Adhere to Google-style docstrings (or adapt to specific company standards).
for example:
```python
class MyClass:
    def visualize_data(self, data: pd.DataFrame, save_path: str = None) -> None:
        Creates and optionally saves a bar chart visualization for the processed data.

        This method uses Altair to create a bar chart comparing average PM2.5 levels by year for
        different cities. If a file path is provided, it will save the chart as a PNG, SVG, or HTML file.
        If no save path is provided, it will display the chart.

        Args:
            data (pd.DataFrame): The DataFrame containing the processed data with columns for 'City', 
                                 'year', and 'Mean' (average PM2.5).
            save_path (str, optional): The file path where the chart should be saved. Supported formats 
                                       include .png, .svg, and .html. Defaults to None.

        Raises:
            ValueError: If the input DataFrame is empty or an unsupported file format is provided.
            Exception: For any unexpected issues during the chart creation or saving process.

        Returns:
            None
```
9. Main module integration: For `main.py`, ensure it orchestrates and utilizes all functions from all modules, preserving the full original notebook functionality.
"""
